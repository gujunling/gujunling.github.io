...
### 2.5 方法（调用时需要使用$.fn.fullpage，例如 $.fn.fullpage.moveTo(1); 或者fullpage_api.getActiveSection();  或者先通过 fullpage = new fullpage('#fullpage', { 。。。}来实例化fullpage对象，再通过fullpage. .moveTo(1);来调用相关方法）
```
     //方式一
     // fullpage插件的事件
