...
`sinon.spy()`是指 根据已有函数 生成一个间谍函数，它会记录下函数调用的参数，返回值，this的值，以及抛出的异常。

`sinon.stub()` 可以说是 spy() 的加强版，他还能额外操作函数的行为。

#### 5.5 mount和shallowMount的区别是什么？

shallowMount仅仅挂载当前组件实例；而mount挂载当前组件实例以外，还会挂载子组件。
...
//具体如何使用请根据自己的实际情况进行安装
```

参考链接：[https://vue-test-utils.vuejs.org/zh/installation/#%E7%94%A8-jest-%E6%B5%8B%E8%AF%95%E5%8D%95%E6%96%87%E4%BB%B6%E7%BB%84%E4%BB%B6](https://vue-test-utils.vuejs.org/zh/installation/#%E7%94%A8-jest-%E6%B5%8B%E8%AF%95%E5%8D%95%E6%96%87%E4%BB%B6%E7%BB%84%E4%BB%B6)
...
module.exports = {
  ...
  // 转换前与所有源文件路径匹配的regexp模式字符串数组。如果文件路径与任何模式匹配，则不会进行转换。
...
describe('======= Infobar - 页面信息栏', () => {
  test('01 不配置数据，标题取自路由', () => {
  ...
  test('05 标题+描述+返回按钮+slot[right]bar', () => {
    const wrapper = shallowMount(Infobar, {
    //组件需要传递的参数
      propsData: Infobar04,
    //mock数据
...
测试`elementUI`组件库中分页组件中点击事件（上一页下一页）是否调用正确
...
const props = {
 ...
 test('03 检查点击上一页/下一页', async () => {
    wrapperM.find('.btn-prev').trigger('click')
    await wrapperM.vm.$nextTick()

    expect(wrapperM.find('.number+.active').text()).toBe(String(props.currentPage - 1))

    wrapperM.find('.btn-next').trigger('click')
    await wrapperM.vm.$nextTick()

    expect(wrapperM.find('.number+.active').text()).toBe(String(props.currentPage - 1 + 1))
  })
  })
```
...
测试结果中，成功的用例会用绿色表示，而失败的部分会显示为红色，若出现如下图所示（没有出现failed），则表示执行测试用例成功。生成快照后会在`index.spec.js`的同级目录下自动创建一个新目录`__snapshots__`，在此目录下存放保存快照所生成的HTML代码。

![](https://cdn.jsdelivr.net/gh/gujunling/PicGo-image/test/20210406153359.png)

若测试用例执行失败，即执行上述命令后出现failed，则根据报错信息查看具体问题，在单元测试中，如果没有通过测试，要么是因为测试的对象有问题，或者是测试代码有问题。

![](https://cdn.jsdelivr.net/gh/gujunling/PicGo-image/test/20210406163100.png)
...
`sinon`github地址：[https://github.com/sinonjs/sinon](https://github.com/sinonjs/sinon)
...
参考博客1：[https://www.jianshu.com/p/3089debd5860](https://www.jianshu.com/p/3089debd5860)

参考博客2：[https://segmentfault.com/a/1190000014112447](https://segmentfault.com/a/1190000014112447)
